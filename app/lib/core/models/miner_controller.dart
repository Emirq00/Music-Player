...
import 'package:music_database/core/mining/miner.dart';
import 'package:music_database/core/models/rola_data.dart';
import 'package:music_database/core/utils/preferences.dart';
import 'package:music_database/core/database/handle_database.dart';
...
class MinerController {
  ...
  Future<List<RolaData>> mineAndSave({String? customPath}) async {
    final musicPath = customPath ?? await Preferences.getPath(); // get dir
    ...
    for (final s in songs) {
      ...
      final performerId = s.performer.trim().toUpperCase() == 'UNKNOWN'
                        ? _unknownPerformerId! : await _db.insertPerformer(idType: 0, name: s.performer);
    ...
    return inserted; // for represent each rola in GUI, a rola row
  }
